...
containsWhereValueIsLessThanLower test case in range checks when the method is given a lower value input that is not in the range it is checking it with. In the last assignment, where we were doing blackbox testing, we tested the contains method with values that were only within the range . In assignment 3 we moved beyond the boundaries and therefore tested what the function would return if we decided to go below the value range. This allowed us to test a part of a function that was not tested in assignment 2 but tested in assignment 3.

combineNormalRange test case in range checks the method combine with standard inputs. The combine method essentially takes 2 ranges and tries to combine them to return an even larger range that includes the original two ranges. The combineNormalRange test does not do anything special and in fact just inputs two standard ranges and checks if the method has executed the operation correctly. In our assignment 2 we did not test for this method originally. This caused our coverage for this case to be 0% for this method in particular. In assignment 3 we visited this method with many test cases including the method described above which allowed us to gain a coverage of 100%.

expandNormal test case in range checks the method expand with standard inputs. The expand method essentially makes a new range by adding margins to the current range. The expandNormal test case does not do anything special and in fact just inputs a standard range and 2 margins. In our assignment 2 we did not test for this method originally which caused our coverage to be 0% for this method in particular. In assignment 3 we were able to visit this method with many test cases including the method described above which allowed us to gain a coverage that was 100%.

shiftNormalTrue test case in range checks the method shift with standard inputs. The shift method essentially shifts the range of the original base range by a particular amount specified in the inputs. The shiftNormalTrue method case from a front perspective does not do anything special but insert just standard inputs and tests if the method returns the correct output; however, it does also increase the coverage beyond the range of just the method itself. It inadvertently checks not only the shift method, but also the overridden shift function as well as the shiftWithZeroCrossing method. All of which were not tested in assignment 2 which caused it to have a 0% coverage. By testing this one function we were able to increase the coverage by around 80%. 

clone_2x2Array test case in DataUtilities checks the method clone with a 2 by 2 matrix. The clone method gets an input array and returns a clone of that array. The clone method was a method that was not tested in our assignment 2 and so it had a coverage cost of 0%. After testing the method with methods such as the one described above we were able to get clone coverage to over 100%.
